...
for OP in OPs:
    ...
    files = np.array(["M_proj/"+f for f in os.listdir("M_proj/") if str(nt) in f])
    ...
    for i in range(len(files)):
        ...
        if labels[i] == 'true':
            plt.errorbar(qtaus,response,yerr=err,label="M = {}".format(labels[i]),color='k')
