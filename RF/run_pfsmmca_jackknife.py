...
for OP in ['M\(tau\)M']:
    ...
    cmd = 'pfsmmca --decorr 1 --jkbins {} calc_qtau_corrmtx '.format(nbin) + OP + ' ' + ' '+ HDF5_file 
    os.system(cmd)
    ...
    for j in range(1,nbin+1):
        if nbin ==1:
            ...
            cmd = 'mv t.mtau_jk{}.db'.format(j) + db_ftr + '.nt' + nt_str + '.smmc.response.corrmtx jk/t.mtau_jk{}.db'.format(j) + db_ftr + '.nt' + nt_str + '.no_blocking.smmc.response.corrmtx'
        os.system(cmd)
        #print(cmd)

 #   cmd = 'mv t.mtau_full.db' + db_ftr + '.nt' + nt_str + '.smmc.response.corrmtx {}_proj/t.mtau_full.db'.format(projType.upper()) + db_ftr + '.nt' + nt_str + '.no_blocking.smmc.response.corrmtx'
 #   os.system(cmd)
...
if nbin > 1:
    for OP in ['M\(tau\)M']:
        ...
        cmd = 'mv t.{}.db'.format(label) + db_ftr + '.nt' + nt_str + '.smmc.response.corrmtx jk/t.{}_true.db'.format(label) + db_ftr + '.nt' + nt_str + '.no_blocking.smmc.response.corrmtx'
        os.system(cmd)
